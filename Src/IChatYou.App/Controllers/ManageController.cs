namespace IChatYou.App.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> RemoveLogin(string loginProvider, string providerKey)
        {
            ...
            var result = await ApplicationUserManager.RemoveLoginAsync(User.Identity.GetUserId(), new UserLoginInfo(loginProvider, providerKey));
            if (result.Succeeded)
            {
        ...
        public async Task<ActionResult> AddPhoneNumber(AddPhoneNumberViewModel model)
        {
            ...
            var code = await ApplicationUserManager.GenerateChangePhoneNumberTokenAsync(User.Identity.GetUserId(), model.Number);
        ...
        {
            await ApplicationUserManager.SetTwoFactorEnabledAsync(User.Identity.GetUserId(), true);
            var user = await ApplicationUserManager.FindByIdAsync(User.Identity.GetUserId());
            if (user != null)
            {
                await ApplicationSignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
        ...
        {
            await ApplicationUserManager.SetTwoFactorEnabledAsync(User.Identity.GetUserId(), false);
            var user = await ApplicationUserManager.FindByIdAsync(User.Identity.GetUserId());
            if (user != null)
            {
                await ApplicationSignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
        ...
        public async Task<ActionResult> VerifyPhoneNumber(VerifyPhoneNumberViewModel model)
        {
            ...
            var result = await ApplicationUserManager.ChangePhoneNumberAsync(User.Identity.GetUserId(), model.PhoneNumber, model.Code);
            if (result.Succeeded)
            {
                var user = await ApplicationUserManager.FindByIdAsync(User.Identity.GetUserId());
                if (user != null)
        ...
        {
            var result = await ApplicationUserManager.SetPhoneNumberAsync(User.Identity.GetUserId(), null);
            if (!result.Succeeded)
            {
        ...
        public async Task<ActionResult> SetPassword(SetPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                var result = await ApplicationUserManager.AddPasswordAsync(User.Identity.GetUserId(), model.NewPassword);
                if (result.Succeeded)
                {
                    var user = await ApplicationUserManager.FindByIdAsync(User.Identity.GetUserId());
                    if (user != null)
        ...
        public async Task<ActionResult> ManageLogins(ManageMessageId? message)
        {
            ...
            var userLogins = await ApplicationUserManager.GetLoginsAsync(User.Identity.GetUserId());
